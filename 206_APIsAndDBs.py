...
>>>>>>> cfd52f9ae98c46c94a00f6bc2e8e1a1288e055d6
try:
...
for user in umich_tweets:
    tweet_info = (user['user']['id'], user['user']['screen_name'], user['user']['favourites_count'], user['user']['description'])
    cur.execute('INSERT or IGNORE INTO Users VALUES (?,?,?,?)', tweet_info)

    for items in user['entities']['user_mentions']:
        user = api.get_user(items['screen_name'])
        tweet_info = (user['id_str'], user['screen_name'], user['favourites_count'], user['description'])
        cur.execute('INSERT OR IGNORE INTO Users VALUES (?,?,?,?)', tweet_info)

for tweet in umich_tweets:
	data1 = (tweet['id_str'], tweet['text'], tweet['user']['id'], tweet['created_at'], tweet['retweet_count'])
	cur.execute('INSERT OR IGNORE INTO Tweets VALUES (?,?,?,?,?)', data1)
